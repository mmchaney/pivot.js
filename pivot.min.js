...
function(){return f.ondevicemotion!==undefined});Modernizr.addTest("webkitmatchesselector",function(){return Element&&!!Element.prototype.webkitMatchesSelector});Modernizr.addTest("webkitcssmatrix",function(){return!!WebKitCSSMatrix});d.availableEvents=["swipeleft","swiperight"];d.prototype={setType:function(c){this.type=c.toLowerCase();this.event=h.createEvent("Events");this.event.initEvent(c,true,false)},dispatchEvent:function(c,a){this.type===a.toLowerCase()&&c.dispatchEvent(this.event)},getCoords:function(c){return{x:c.touches[0].pageX,
y:c.touches[0].pageY}},onTouchStart:function(c){if(this.startCoords=this.getCoords(c)){this.element.addEventListener("touchend",this.onTouchEnd,false);this.element.addEventListener("touchmove",this.onTouchMove,false)}},onTouchMove:function(c){this.moveCoords=this.getCoords(c)},onTouchEnd:function(c){var a;this.element.removeEventListener("touchend",this.onTouchEnd,false);this.element.removeEventListener("touchmove",this.onTouchMove,false);if(this.moveCoords){a={x:this.startCoords.x-this.moveCoords.x,
y:this.startCoords.y-this.moveCoords.y};if(Math.abs(a.x)>30||Math.abs(a.y)>30)this.dispatchEvent(c.target,a.x>0?"swipeleft":"swiperight");delete this.moveCoords}}};g.util.makeElement=function(c,a){var b=h.createElement(c),e;for(e in a)if(e==="class")b.className=a[e];else b.setAttribute(e,a[e]);return b};g.util.delegate=function(c,a,b,e){c.addEventListener(b,function(j){for(var i=j.target,k=g.util.slice.call(c.querySelectorAll(a));i&&k.indexOf(i)<0;)i=i.parentNode;i&&i!==c&&i!==h&&e.call(i,j)},false)};
g.util.ancestor=function(c,a){for(;c!==h&&!c.webkitMatchesSelector(a);)c=c.parentNode;return c};g.util.supplant=function(c){var a=g.util.slice.call(arguments),b=0,e;return typeof a[1]==="object"?c.replace(/\{(\w+)\}/g,function(j,i){return a[1][i]!==undefined?a[1][i]:j}):c.replace(/\{(\w*)\}/g,function(j){e=a[++b];return e!==undefined?e:j})};g.util.getJSON=function(c,a){var b=new XMLHttpRequest;b.addEventListener("readystatechange",function(){b.readyState===4&&b.status===200&&a(JSON.parse(b.responseText))},
false);b.open("GET",c,true);b.send()};g.util.localCoordinates=function(c,a){var b;a=a||c.target;b=a.getBoundingClientRect();return{x:Math.max(0,Math.min(1,(c.clientX-b.left)/b.width)),y:Math.max(0,Math.min(1,(c.clientY-b.top)/b.height)),box:b}};g.util.mod=function(c,a){return(c%a+a)%a};g.util.gesturize=function(c,a,b){return new d(c,a,b)};g.util.preventTouchScroll=function(c){c.addEventListener("touchmove",function(a){a.preventDefault()},false)};Function.prototype.bind=Function.prototype.bind||function(c){var a=
this,b=g.util.slice.call(arguments,1);return function(){return a.apply(c,b.concat(g.util.slice.call(arguments)))}};if(Object.prototype.__defineGetter__&&!Object.defineProperty)Object.defineProperty=function(c,a,b){"get"in b&&c.__defineGetter__(a,b.get);"set"in b&&c.__defineSetter__(a,b.set)};typeof Element!=="undefined"&&!Element.prototype.classList&&function(){var c=function(b){return RegExp("(^|\\s)"+b+"(\\s|$)")},a=function(b){this.element=b};a.prototype={contains:function(b){return c(b).test(this.element.className)},
