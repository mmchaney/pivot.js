(function(e){function d(a,b,c){this.element=a;this.setType(b);this.element.addEventListener(b,c,false);this.onTouchEnd=this.onTouchEnd.bind(this);this.onTouchMove=this.onTouchMove.bind(this);this.onTouchStart=this.onTouchStart.bind(this);this.element.addEventListener("touchstart",this.onTouchStart,false)}var f=e.pivot||(e.pivot={}),h=e.document;f.util={};f.util.slice=[].slice;Modernizr.addTest("skia",function(){return/Win/.test(e.navigator.platform)&&/Chrome/.test(e.navigator.userAgent)});Modernizr.addTest("devicemotion",
function(){return e.ondevicemotion!==undefined});Modernizr.addTest("webkitmatchesselector",function(){return Element&&!!Element.prototype.webkitMatchesSelector});Modernizr.addTest("webkitcssmatrix",function(){return!!e.WebKitCSSMatrix});d.availableEvents=["swipeleft","swiperight"];d.prototype={setType:function(a){this.type=a.toLowerCase();this.event=h.createEvent("Events");this.event.initEvent(a,true,false)},dispatchEvent:function(a,b){this.type===b.toLowerCase()&&a.dispatchEvent(this.event)},getCoords:function(a){return{x:a.touches[0].pageX,
...
y:this.startCoords.y-this.moveCoords.y};if(Math.abs(b.x)>30||Math.abs(b.y)>30)this.dispatchEvent(a.target,b.x>0?"swipeleft":"swiperight");delete this.moveCoords}}};f.util.makeElement=function(a,b){var c=h.createElement(a),g;for(g in b)if(g==="class")c.className=b[g];else b.hasOwnProperty(g)&&c.setAttribute(g,b[g]);return c};f.util.delegate=function(a,b,c,g){a.addEventListener(c,function(j){for(var i=j.target,k=f.util.slice.call(a.querySelectorAll(b));i&&k.indexOf(i)<0;)i=i.parentNode;i&&i!==a&&i!==
h&&g.call(i,j)},false)};f.util.ancestor=function(a,b){for(;a!==h&&!a.webkitMatchesSelector(b);)a=a.parentNode;return a};f.util.supplant=function(a){var b=f.util.slice.call(arguments),c=0,g;return typeof b[1]==="object"?a.replace(/\{(\w+)\}/g,function(j,i){return b[1][i]!==undefined?b[1][i]:j}):a.replace(/\{(\w*)\}/g,function(j){g=b[++c];return g!==undefined?g:j})};f.util.getJSON=function(a,b){var c=new XMLHttpRequest;c.addEventListener("readystatechange",function(){c.readyState===4&&c.status===200&&
b(JSON.parse(c.responseText))},false);c.open("GET",a,true);c.send()};f.util.localCoordinates=function(a,b){var c;b=b||a.target;c=b.getBoundingClientRect();return{x:Math.max(0,Math.min(1,(a.clientX-c.left)/c.width)),y:Math.max(0,Math.min(1,(a.clientY-c.top)/c.height)),box:c}};f.util.mod=function(a,b){return(a%b+b)%b};f.util.gesturize=function(a,b,c){return new d(a,b,c)};f.util.preventTouchScroll=function(a){a.addEventListener("touchmove",function(b){b.preventDefault()},false)};Function.prototype.bind=
Function.prototype.bind||function(a){var b=this,c=f.util.slice.call(arguments,1);return function(){return b.apply(a,c.concat(f.util.slice.call(arguments)))}};if(Object.prototype.__defineGetter__&&!Object.defineProperty)Object.defineProperty=function(a,b,c){"get"in c&&a.__defineGetter__(b,c.get);"set"in c&&a.__defineSetter__(b,c.set)};typeof Element!=="undefined"&&!Element.hasOwnProperty.call(Element.prototype,"classList")&&function(){var a=function(c){return RegExp("(^|\\s)"+c+"(\\s|$)")},b=function(c){this.element=
c};b.prototype={contains:function(c){return a(c).test(this.element.className)},add:function(c){this.contains(c)||(this.element.className+=(this.element.className?" ":"")+c)},remove:function(c){this.element.className=this.element.className.replace(a(c)," ").trim()},toggle:function(c){var g=a(c);if(g.test(this.element.className))this.element.className=this.element.className.replace(g," ").trim();else this.element.className+=(this.element.className?" ":"")+c}};Object.defineProperty(Element.prototype,
"classList",{get:function(){return new b(this)}})}()})(this);
(function(e){var d=e.pivot||(e.pivot={}),f=e.document,h=d.util.supplant;d.Gallery=function(a){var b;this.wrapper=a.wrapper||f.body;this.quality=a.quality||"medium";this.rows=a.rows;this.columns=a.columns;this.page=1;this.perPage=this.rows*this.columns;b={perPage:this.rows*this.columns};if(a.userId){this.feed=d.flickr.feeds.user;b.userId=a.userId}else if(a.groupId){this.feed=d.flickr.feeds.group;b.groupId=a.groupId}else this.feed=d.flickr.feeds.interesting;this.feed=h(this.feed,b);this.photoLoadCount=
...
new WebKitCSSMatrix:undefined,constrainLayout:function(){var a=this.container.offsetWidth,b=this.container.offsetHeight;this.zoomPlane.style.width=this.zoomPlane.style.height=Math.min(a,b)+"px";if(a>b){this.zoomPlane.style.left=(a-b)/2+"px";this.zoomPlane.style.top=0}else{this.zoomPlane.style.top=(b-a)/2+"px";this.zoomPlane.style.left=0}},getPhoto:function(a,b){return this.container.querySelector(h('.p-photo[data-row="{row}"][data-column="{column}"]',a,b))},getSelectedPhoto:function(){return this.container.querySelector(".p-photo.selected")},
...
if(this.zoomed){a=this.getSelectedPhoto();a=h("translate3d({x}%, {y}%, -150px)",{x:a.getAttribute("data-column")*-100,y:a.getAttribute("data-row")*-100})}else a=h("translate3d({x}%, {y}%, 1px)",{x:this.columns/2*-100+50,y:this.rows/2*-100+50});this.trackingPlane.style.webkitTransform=a},tilt:function(){var a=this.zoomed&&e.orientation===undefined?this.allowedRotation/4:this.allowedRotation;this.tilting=true;this.rotationDelta.x=(a*(this.movementCoordinates.y-0.5)-this.currentRotation.x)/this.tiltLag;
